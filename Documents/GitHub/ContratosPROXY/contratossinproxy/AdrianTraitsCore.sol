...
contract AdrianTraitsCore is ERC1155, Ownable, ReentrancyGuard {
    ...
    address public assetRegistrar;  // Nuevo: referencia al registrar
    ...
    event AssetRegistrarUpdated(address newRegistrar);  // Nuevo evento
    ...
    modifier onlyRegistrar() {
        require(msg.sender == assetRegistrar, "Not registrar");
        _;
    }
    ...
    /**
     * @dev Set asset registrar contract
     */
    function setAssetRegistrar(address _addr) external onlyOwner {
        assetRegistrar = _addr;
        emit AssetRegistrarUpdated(_addr);
    }

    /**
     * @dev Import assets from registrar
     */
    function importFromRegistrar(uint256[] calldata ids) external onlyOwner {
        require(assetRegistrar != address(0), "!registrar");
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            (
                ...
                bool tempFlag,  // Renombrado de isTemporary a tempFlag
                uint256 maxSupply,
                uint256 assetType
            ) = IAssetRegistrar(assetRegistrar).assets(ids[i]);

            uint256 assetId = nextAssetId++;
            
            // Validación de rangos de ID
            if (AssetType(assetType) == AssetType.PACK) {
            ...
            emit AssetRegistered(assetId, name, category, AssetType(assetType));
        }
    }
    ...
    /**
     * @dev Burn trait (Extensions contract or Serum module)
     */
    function burn(address account, uint256 id, uint256 amount) external {
        require(msg.sender == extensionsContract || msg.sender == serumModule, "!auth");
        _burn(account, id, amount);
    }

    /**
     * @dev Set asset metadata (solo para el registrar)
     */
    function registrarSetAssetMetadata(uint256 assetId, string calldata newUri) external onlyRegistrar {
        require(assetId < nextAssetId, "Asset does not exist");
        assets[assetId].metadata = newUri;
        emit AssetURIUpdated(assetId, newUri);
    }

    /**
     * @dev Permite cambiar la categoría de un trait
     */
    function setTraitCategory(uint256 traitId, string calldata newCategory) external onlyOwner {
        assets[traitId].category = newCategory;
    }
