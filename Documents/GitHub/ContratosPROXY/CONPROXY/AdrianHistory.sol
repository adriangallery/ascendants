 ...
 */
contract AdrianHistory is AdrianStorage, ReentrancyGuard, Initializable {
    ...
    function initialize(address _proxy) external {
    ...
    event HistoryWriterAdded(address writer);
    event HistoryWriterRemoved(address writer);
    event HistoryEventRecorded(uint256 indexed tokenId, bytes32 eventType, address actor);
    event HistoryEventDeleted(uint256 indexed tokenId, uint256 eventIndex);
    event HistoryCleared(uint256 indexed tokenId);

    // =============== Modifiers ===============
    
    modifier onlyHistoryWriter() {
        require(historyWriters[msg.sender], "!writer");
        _;
    }
     ...
     */
    function setHistoryWriter(address writer, bool authorized) external onlyProxy {
