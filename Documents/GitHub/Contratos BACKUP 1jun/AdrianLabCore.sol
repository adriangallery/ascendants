...
contract AdrianLabCore is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function applyMutationFromSerum(
        ...
        // Registrar historia si existe el contrato de extensiones
        if (extensionsContract != address(0)) {
            bytes memory eventData = abi.encode(narrativeText, uint256(0));
            IAdrianLabExtensions(extensionsContract).recordHistory(
                tokenId,
                keccak256(bytes("MUTATION")),
                eventData
            );
        }
    }
    ...
    function replicate(uint256 tokenId) external nonReentrant {
        ...
        if (mutationLevel[newTokenId] == MutationType.NONE || mutationLevel[newTokenId] == MutationType.MILD) {
            if (_randomChance(50)) {
    ...
    function _selectRandomSkin() internal view returns (uint256) {
        uint256 rand = _random(block.timestamp + tokenCounter, totalSkinWeight);
    ...
    /**
     * @dev Duplicate GEN0 tokens
     */
    function duplicateGen0Tokens(uint256[] calldata tokenIds) external onlyOwner nonReentrant {
