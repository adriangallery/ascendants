...
contract AdrianTraitsExtensions is Ownable, ReentrancyGuard {
    ...
    function _traitAlreadyEquipped(uint256 tokenId, uint256 traitId) internal view returns (bool) {
        string memory category = traitsCore.getCategory(traitId);
        return tokenTraits[tokenId][category] == traitId;
    }
    ...
    interface IAdrianTraitsExtensions {
        ...
        function getTokenTraits(uint256 tokenId) external view returns (string[] memory, uint256[] memory);
    }
}
...
interface IAdrianTraitsCore {
    ...
    function getTraitInfo(uint256) external view returns (string memory, bool);
}
