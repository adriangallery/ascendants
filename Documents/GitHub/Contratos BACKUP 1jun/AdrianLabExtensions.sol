...
contract AdrianLabExtensions is Ownable, ReentrancyGuard {
    ...
    function setCoreContract(address _core) external onlyOwner {
        require(_core != address(0) && _core.code.length > 0, "Invalid contract address");
        coreContract = _core;

        // Llamada bidireccional si el Core lo permite
        try IAdrianLabCore(_core).setExtensionsContract(address(this)) {
            // Ok
        } catch {
            revert("Failed to set back reference in Core");
        }
    }
     ...
     */
    function setAdminContract(address _contract) external onlyOwner {
        require(_contract != address(0) && _contract.code.length > 0, "Invalid contract");
        adminContract = _contract;
    }
...
interface IAdrianLabCore {
    ...
    function setExtensionsContract(address _ext) external;
    function setAdminContract(address _admin) external;
}
