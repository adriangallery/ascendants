...
interface IAdrianTraitsCore is IERC1155 {
    ...
    /// @notice Devuelve categoría y flag de "temporal" para un trait dado
    /// @param assetId ID del trait/asset
    /// @return category La categoría del trait
    /// @return isTemp Flag que indica si el trait es temporal
    function getTraitInfo(uint256 assetId) external view returns (string memory category, bool isTemp);

    /// @notice Lee la categoría de un asset (trait) dado su ID
    /// @param assetId  ID del trait/asset
    /// @return         la categoría (string) almacenada en `assets[assetId].category`
    function getCategory(uint256 assetId) external view returns (string memory);

    /// @notice Firma para comprar (reservar) packs en el core
    /// @param packId      ID del pack a comprar
    /// @param quantity    cantidad de packs a reservar
    /// @param merkleProof prueba Merkle si el pack está en allowlist
    ...
    /// @notice Firma para abrir packs ya reservados en el core
    /// @param packId ID del pack a abrir
